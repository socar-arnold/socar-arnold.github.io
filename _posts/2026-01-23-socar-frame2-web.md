...
쏘카에서 장기간의 프로젝트인 쏘카프레임 2.0 개발에 참여하며 고민하고 개발하였던 이야기를 해보려 합니다. 이 글에서 쏘카프레임은 쏘카의 사내 UI 라이브러리이자 디자인 시스템입니다. 이 글은 웹 디자인 시스템(프론트엔드) 관점의 설계와 구현에 집중합니다.

> 각 파트별로 할 수 있는 이야기가 많지만 본 글에서는 시스템의 큰 방향과 약간의 세부사항에 대한 이야기를 적어보겠습니다.

쏘카에는 기존의 UI 라이브러리인 **쏘카프레임**이 존재하였습니다.

기존 UI 라이브러리는 디자인 시스템을 표방했습니다. 하지만 입사 당시 이미 라이브러리의 R&R(Role & Responsibility)이 모호한 상태였습니다. 유지보수가 거의 되지 않았고, 각 서비스별로 파편화된 UI/UX 개발이 이뤄지고 있는 상황이었습니다.
...
쏘카는 사내 디자인 툴로 Figma를 활용하고 있었습니다. 디자인 산출물을 개발자가 한땀한땀 구현으로 옮기는 것이 기존 업무 방식이었습니다.
...
![BottomSheet 훅 구성 흐름](/img/2026-01-23-socar-frame2-web/bottomsheet-hooks.png)
...
사내 LLM 서비스에 Figma MCP(Model Context Protocol)를 접목하면, 에디터 내에서 특정 노드 ID를 기준으로 `디자이너 → 개발자(서비스)` 흐름을 더 직접적으로 만들 수 있습니다.
