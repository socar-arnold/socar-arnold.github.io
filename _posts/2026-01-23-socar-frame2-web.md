---
layout: post
title: "쏘카 디자인 시스템 개발기 1편: 시스템으로 굴리기(웹)"
...
쏘카에서 장기간의 프로젝트인 쏘카프레임 2.0 개발에 참여하며 고민하고 개발하였던 이야기를 해보려 합니다. 이 글에서 쏘카프레임은 쏘카의 사내 UI 라이브러리이자 디자인 시스템입니다. 이 글은 웹 디자인 시스템(프론트엔드) 관점의 **시스템 설계/연동/운영**에 집중합니다.

이 시리즈는 1편과 2편으로 나뉩니다. 1편에서는 시스템 설계, Figma 연동, 운영 프로세스를 다루고, 2편에서는 컴포넌트 아키텍처, 패키지 전략, AI 활용을 중심으로 정리합니다.
...
이 글에서 말하는 “시스템”은 **컴포넌트 라이브러리만**을 의미하지 않습니다. UI 라이브러리 위에 **정책(규칙), 연동(디자인-코드), 운영(릴리스/검증)**이 얹혀 있어야 실제로 반복 가능한 생산 체계가 됩니다. 즉, 구현물과 함께 **합의된 규칙과 운영 방식까지 포함**될 때 비로소 시스템으로 작동한다고 정의했습니다.

간단히 정리하면 아래 4가지가 함께 움직여야 했습니다.
...
범위도 명확히 했습니다. 디자인 토큰, 컴포넌트, 문서, 연동 규칙은 시스템의 범위로 포함했지만, 서비스별 UI 정책과 화면 구성은 각 서비스의 책임으로 분리했습니다. 이렇게 경계를 정해야 역할이 섞이지 않고, 시스템이 “공통 기반”으로 기능할 수 있다고 봤습니다.

# 2. 시스템 설계

## 2.1 설계 목적

기존의 사례가 존재했기 때문에 변화에 대응하고 확장 가능한 UI 라이브러리를 만드는 것이 목표였습니다.

다만 UI 라이브러리만으로는 생산성 개선이 제한적이라 판단했습니다. 그래서 **라이브러리 + 연동 체계 + 운영 규칙**을 함께 설계하는, 시스템으로서의 구조를 고민했습니다.

쏘카는 사내 디자인 툴로 Figma를 활용하고 있었습니다. 디자인 산출물을 개발자가 한땀한땀 구현으로 옮기는 것이 기존 업무 방식이었습니다. 이 흐름을 **연동 체계 강화**로 전환하고자 했습니다.

구체적으로는 `디자이너 → 개발자(서비스)` 전달 과정과 `개발자(라이브러리) → 개발자(서비스)` 전달 과정을 최적화하는 데 초점을 두었습니다. 이 과정은 Figma의 지원 기능을 적극 활용해 설계했습니다.
...
기존에는 아이콘·스페이싱 토큰이 슬랙 → 담당 개발자 수동 PR(Pull Request) 생성 → 검증 → 배포로 이어져 리드타임이 길었습니다.
...
운영 효과를 검증하기 위한 정량 지표 후보도 정리했습니다. 토큰/아이콘 반영 리드타임, 컴포넌트 릴리스 후 서비스 반영 리드타임, 핸드오프 질의 건수 등을 추후 측정할 계획입니다.
...
각 파트별로 정말 많은 이야기를 나눌 수 있는 주제들이지만, 이 글에서는 시스템으로 작동하기 위한 설계와 운영 관점 위주로 공유했습니다.
세부 구현까지 모두 담기엔 부족하지만, 그만큼 넓게 봤던 고민과 경험을 기록해 두고 싶었습니다.

특히 Figma 연동을 통해 `디자이너 → 개발자` 흐름과 `개발자(라이브러리) → 개발자(서비스)` 흐름을 동시에 정렬하려 했던 시도가 가장 큰 전환점이었습니다.
단순한 도구 연동을 넘어, **설계 방식과 운영 규칙을 합의**하는 과정이 시스템의 기반이 된다고 느꼈습니다.

2편에서는 컴포넌트 아키텍처, 패키지 전략, AI 활용 사례를 중심으로 좀 더 기술적인 내용을 정리해 보겠습니다. [2편 바로가기](/fe/2026/01/24/socar-frame2-web-part2.html)
